...
Be specific — name actual policies, dates, sectors. No filler language."""
            ...
            cache_key = f"gemini_explain_{selected_year}"
            if cache_key in st.session_state:
                answer = st.session_state[cache_key]
                st.markdown(f'<div class="ai-response">{answer}</div>', unsafe_allow_html=True)
                st.caption("Generated by Gemini 2.0 Flash (cached) | Data: RBI Handbook of Statistics 2024-25")
            else:
                with st.spinner("Gemini is analysing this year's macro context..."):
                    try:
                        ...
                        st.markdown(f'<div class="ai-response">{answer}</div>', unsafe_allow_html=True)
                        st.caption("Generated by Gemini 2.0 Flash | Data: RBI Handbook of Statistics 2024-25")
                    except requests.exceptions.HTTPError as e:
                        code = e.response.status_code
                        if code == 429:
                            st.warning("Rate limit hit — please wait 30 seconds and try again. Gemini free tier allows ~15 requests/minute.")
                        elif code == 503:
                            st.warning("Gemini's servers are temporarily overloaded (503). This isn't a key issue — please try again in a moment.")
                        elif code == 404:
                            st.error("API error: 404 — the model name or endpoint is invalid, not the key.")
                        elif code in (401, 403):
                            st.error(f"API error: {code} — your API key is invalid, disabled, or lacks access.")
                        else:
